...
class TMail_Compat extends AbstractController
{
    ...
    public function loadTemplate($template, $type = '.txt')
    {
    ...
    public function setTag($tag, $value = null)
    {
        ...
        foreach ($this->attrs as $key => $attr) {
            if ($attr instanceof SMlite) {
                $this->get($key)->trySet($tag, $value);
            }
        }
        if ($this->get('subject') instanceof SMlite) {
            $this->get('subject')->trySet($tag, $value);
        }
        if ($this->body instanceof SMlite) {
            $this->body->trySet($tag, $value);
        }
        if ($this->sign instanceof SMlite) {
            $this->sign->trySet($tag, $value);
        }

        return $this;
    }
    public function setIsHtml($is_html = true)
    {
    ...
    public function loadDefaultTemplate()
    {
        ...
        if ($this->is_html) {
            $template->set('content_type', 'text/html');
        }
    ...
    public function get($tag, $plain = true)
    {
        $value = null;
        if (isset($this->attrs[$tag])) {
            $value = $this->attrs[$tag];
        } else {
            ...
            switch ($tag) {
                ...
                case 'errors-to':
                    ...
                    break;
                case 'bcc':
                    $value = false; // not set by default
                    break;
                case 'content-type':
                    if (count($this->mime) > 0) {
                        $value = 'multipart/mixed; boundary='.$this->getBoundary();
                    } elseif ($this->body_type == 'html') {
     ...
     */
    public function attachHTML($html)
    {
    ...
    public function send($address, $add_params = null)
    {
        ...
        if (!$this->get('from')) {
            $this->set('from', $this->app->getConfig('mail/from', 'nobody@agiletoolkit.org'));
        }
