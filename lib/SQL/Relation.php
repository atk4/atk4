...
class SQL_Relation extends AbstractModel
{
    ...
    public function afterInsert($m, $id)
    {
        if ($this->delete_behaviour == 'ignore') {
            return;
        }
    ...
    public function beforeModify($m, $q)
    {
        if ($this->dsql->args['set']) {
            $this->dsql->where($this->f2, $this->id)->update();
        }
    }
    public function beforeDelete($m, $id)
    {
