...
abstract class AbstractObject {
    ...
    function add($class,$short_name=null,$template_spot=null,$template_branch=null,$debug=null){
        ...
        if(is_object($class)){
            ...
            if($this->elements[$class->short_name])return $this->elements[$class->short_name];
    ...
    function learn($name, $value1=null, $value2=null, $value3=null){
        ...
        if(isset($value1))return $this->memorize($name,$value1);
        if(isset($value2))return $this->memorize($name,$value2);
    ...
    function getElement($short_name,$obligatory=true){
        if(!isset($this->elements[$short_name]))
        	if($obligatory)throw new BaseException($this->__toString()." does not have child $short_name");
        	else return null;
        return $this->elements[$short_name];
    }
    ...
    function fatal($error,$shift=0){
        ...
        return $this->upCall('outputFatal',array($error,$shift));
    }
    function info($msg){
        /**
    ...
    function debug($msg, $file=null, $line=null){
         ...
         */
        if(
           (isset($this->debug) && $this->debug) ||
           (isset($this->api->debug) && $this->api->debug)
           ){
            $this->upCall('outputDebug',array($msg,$file,$line));
        }
    }
    function warning($msg,$shift=0){
        $this->upCall('outputWarning',array($msg,$shift));
    }
                ...
                foreach($this->hooks[$hook_spot] as $prio=>$_data){
                    foreach($_data as $data){
                        ...
                        if(is_string($data) && !preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/',$data)){
                            ...
                            if(!is_array($data))$data=array('STATIC',$data);
                            throw new BaseException(sprintf(
                                        "Cannot call %s->%s. Ensure that function exist.",
                                        $data[0],$data[1]));
                        }
    ...
    function hook_wrap($method_name,$hook_spot=null){
        if(!isset($hook_spot))$hook_spot=$method_name;
        $result = $this->hook($hook_spot.'-pre');
        if(!isset($result))$result=$this->$method_name();
